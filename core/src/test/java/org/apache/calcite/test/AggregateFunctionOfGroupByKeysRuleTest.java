...
import static org.apache.calcite.rel.rules.CoreRules.AGGREGATE_FUNCTION_OF_GROUP_BY_KEYS;

/**
 * Unit tests for {@link org.apache.calcite.rel.rules.AggregateFunctionOfGroupByKeysRule}.
 *
 * <p>Relevant tickets:
 * <ul>
 * <li><a href="https://issues.apache.org/jira/browse/CALCITE-7484">
 * [CALCITE-7484] Add AggregateFunctionOfGroupByKeysRule to eliminate redundant
 * aggregates over GROUP BY keys
 * </a></li>
 * </ul>
 */
class AggregateFunctionOfGroupByKeysRuleTest {

  private static RelOptFixture fixture() {
    return RelOptFixture.DEFAULT.withDiffRepos(
        DiffRepository.lookup(AggregateFunctionOfGroupByKeysRuleTest.class));
  }
  ...
  @Test void testAggregateFunctionOfGroupByKeys() {
    ...
    sql(sql).withRule(AGGREGATE_FUNCTION_OF_GROUP_BY_KEYS).check();
  }
  ...
  @Test void testAggregateFunctionOfGroupByKeysNoChange() {
    String sql = "select sal, max(comm) as comm_max\n"
        + "from emp group by sal, deptno";
    sql(sql).withRule(AGGREGATE_FUNCTION_OF_GROUP_BY_KEYS).checkUnchanged();
  }
